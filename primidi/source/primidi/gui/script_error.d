...
/// Error popup for compilation or runtime problems.
final class ScriptErrorModal : GuiElement {
    ...
    this(GrError error) {
        ...
        { //Error display
            ...
            string line = "{c:red}error:{c:black} " ~ error.message ~ "\n" ~ space
                ~ "{c:blue}->{c:black} " ~ error.filePath ~ "(" ~ to!string(
                        error.line) ~ "," ~ to!string(error.column) ~ ")\n{c:blue}" ~ extra ~ "\n" ~ snippet ~ "\n"
    ...
    /// Runtime error
    this(string message, const(GrStackTrace[]) stackTraces) {
        setAlign(GuiAlignX.center, GuiAlignY.center);
        isMovable(true);
        ...
        {
            string line = "{c:red}panic:{c:black} " ~ message ~ "\n\n";
            foreach (trace; stackTraces) {
                line ~= "[" ~ to!string(
                        trace.pc) ~ "] in {c:blue}" ~ trace.name ~ "{c:black} at " ~ trace.file ~ "(" ~ to!string(
    ...
    override void update(float deltaTime) {
        if (getButtonDown(KeyButton.escape) || getButtonDown(KeyButton.enter)
                || getButtonDown(KeyButton.enter2))
            onCallback("close");
    }

    override void draw() {
        drawFilledRect(origin, size, Color(240, 240, 240));
        drawFilledRect(_text.origin - Vec2f(10f, 10f), _text.size + Vec2f(20f,
                20f), Color(204, 204, 204));
    }
