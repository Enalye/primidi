		...
		foreach(octave; 0.. 11) {
			...
			foreach(note; 0.. 7) {
				...
				if(notesState[octave * 12 + noteIndex]) {
					_whiteNoteSprite.color = Color.cyan;
					...
					_whiteNoteSprite.color = Color.white;
					_whiteNoteSprite.draw(notePosition);
				}
				notePosition.x += 10f;
			}
			...
			foreach(note; 0.. 5) {
				...
				if(notesState[octave * 12 + noteIndex]) {
					_blackNoteSprite.color = Color.cyan;
					...
					_blackNoteSprite.color = Color.black;
					_blackNoteSprite.draw(finalPosition);
				}
			}
		}
